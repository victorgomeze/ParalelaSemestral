...
float MDC[16][16]; // Matriz de sitancias que no cambia nunca
int PJ[16][16]; // matriz de 0's y 1's que indica 0= partido no jugado, 1= partido jugado
for (int i=0; i<16;i++){
...
float vecaux[16]; //vector aux

for (int i=0; i<16;i++){
	float aux=99999;
	for (int j=0;j<16;j++){
		
		if(MD[i][j]!=0 && MD[i][j]<aux){
			vecaux[i]=MD[i][j];
			aux=MD[i][j];
		} 
	}
